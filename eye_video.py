...
model_xml = "face-detection-adas-0001.xml" # 学習モデルの呼び出し
model_bin = "face-detection-adas-0001.bin" # xmlに対応するbinファイル
...
v =  cv2.VideoCapture(0) #cameraからキャプチャー
while True:
    ret, frame = v.read() # フレームの読み込み
    if ret == False: break # フレームがなくなった
    if frame.shape[:-1] != (w ,h): # 大きさが違うときリサイズ
        frame = cv2.resize(frame, (w, h))

    image = frame.transpose((2, 0, 1))  # 入力画像がHWC なのをCHWの変換
    ...
    cv2.imshow('image', frame) # フレームの表示

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'): break # qを押したら終了
